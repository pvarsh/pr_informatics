...
FIELDS = {}
FIELDS[1] = ["nyc_ff_jan18"]
FIELDS[2] = ["name"]
FIELDS[3] = ["broadway_ff_avg_diff"]
FIELDS[4] = ["station_with_largest_increase"]
FIELDS[5] = ["station_with_largest_decrease"]
...
def createExpectedResult(cursor, problem, mustSort):
  filename = "query" + str(problem) + "_solution.sql"
  query = ""
  with open(filename) as f:
    query = f.read()
